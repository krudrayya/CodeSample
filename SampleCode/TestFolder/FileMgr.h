	 ...
	 void search() {
		 try{
			 for (auto patternName : patterns_) {
				 ...
				 std::vector<std::string> Directrylist = FileSystem::Directory::getDirectories(path_, "*.*");

				 for (auto paths : Directrylist)
				 {
					 if (paths != "." && paths != "..")
						 store_.saveset(FileSystem::Path::getFullFileSpec(path_) + "\\" + paths);

				 }
				 std::set<std::string> pathy;
				 for (auto f : files) {
